...
{	
	...
	/*public function addPiece(_slot:Int):Void
	{
		var piece:PuzzlePiece = puzzleGroup.recycle(PuzzlePiece);
		piece.launch(null, this);
		...
		mainArray[_slot] = piece;
		
		piece.startingPoint.x = piece.x;
		piece.startingPoint.y = piece.y;
		
		piece.destinationPoint.x = location.x;
		piece.destinationPoint.y = location.y + piece.height * _slot + spaceBuffer * _slot;		
	}*/
	...
	{		
		...
		// Check the Mainarray in reverse order for the first instance of a null. If one is found, then takeRowMembers is set to true,and we know from what point we need to start accounting for any and all remaining puzzle pieces to shift ahead.
		...
		// If takeRowMembers is false, that means no empty/null member of mainArray was found, so we don't need to populate anything after all. Ergo, return out of this function. Let's trace our result just for the hell of it.
		...
		// If, however, takeRowMembers was triggered, that means we have some work to do. In particular, we're going to check the mainArray in order. Any PuzzlePieces we find will be added to the end of rowMembers in the order found. So, pieces occupying spaces 7 5 and 3 would ultimately be added as 7, 5, and 3. That way we can move everything in order when we move them downwards. We also remove them from the mainArray so they can be re-added in order after moving.
		...
		//Now, we tween each of the members of rowMembers into position, starting from rowFillStart and working backwards. What's more, we add the pieces to their new position in the row. This may still leave some empty row spaces left over.
		...
		if (rowMembers.length > 0)		
		{
			for (i in 0...rowMembers.length)
			{
				...
				//var tween:FlxTween = FlxTween.linearMotion(puzzlePiece, puzzlePiece.x, puzzlePiece.y, puzzlePiece.destinationPoint.x, puzzlePiece.destinationPoint.y, refillSpeed, false, {complete: onComplete});		
				
				rowFillStart -= 1;
				
				//Adding this so we know to check this when all is said and done.
				//trace("rowFillStart: " + rowFillStart);
			}
		}
		...
		while (rowFillStart >= 0)
		{
			...
			puzzlePiece.startingPoint.y = location.y - (puzzlePiece.height * newPieceCount);
	...
	private function animatePieces(_array:Array<PuzzlePiece>):Void
	{
		...
		{
			var tween:FlxTween = FlxTween.cubicMotion(_array[i], _array[i].startingPoint.x, _array[i].startingPoint.y, FlxRandom.floatRanged(0, FlxG.width), FlxRandom.floatRanged(0, FlxG.height), FlxRandom.floatRanged(0, FlxG.width), FlxRandom.floatRanged(0, FlxG.height), _array[i].destinationPoint.x, _array[i].destinationPoint.y, .5, { complete: onComplete });
		}
	}
	...
	public function removePiece(t:PuzzlePiece, _fast:Bool = false):Void
	{
		...
		var tween:FlxTween = FlxTween.multiVar(t.scale, { x: .1, y: .1 }, .25, { complete: removePieceTween } );
		tween.userData = t;		
