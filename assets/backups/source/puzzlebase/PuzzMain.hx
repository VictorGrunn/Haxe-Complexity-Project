...
class PuzzMain extends FlxGroup
{
	...
	public var columnAmount:Int = 10;
	public var rowAmount:Int = 10;
	...
	private function startUsOff():Void
	{
		var initialize:PuzzleInitializeProcess = new PuzzleInitializeProcess(this);
		rules.addToQueue(initialize);
	}
	...
	/*
	 * Generated the initial puzzle. No matches/duplicates included.
	 */
	...
	/*
	 * Runs through the puzzle list, checking for solutions - ie, seeing if there's any single move that can result in a match.
	 */
	...
	{		
		...
		{
			for (i in 0...solutionArray.length)
			{
				solutionArray[i].alpha = .5;
			}
			trace(solutionPairs.length + " solutions found.");					
	...
	{	
		if (orderQueue != 0)
		{
			...
			{
				//checkMatches();			
				//var matchCheck:MatchCheckProcess = new MatchCheckProcess(this);
				//rules.addToQueue(matchCheck);
			}
		}
		else
		{
	...
	{
		var newProcess:MatchCheckProcess = new MatchCheckProcess(this);
		rules.addToQueue(newProcess);		
	}
	
	//Cleans and populates rows.
	public function checkRows():Void
	{
		var rowChecker:RefillRowsProcess = new RefillRowsProcess(this);
		rules.addToQueue(rowChecker);
		/*for (i in 0...columnArray.length)
		{
	...
	override public function update():Void
	{
		...
		if (FlxG.keyboard.justPressed("F"))
		{
			//areThereSolutions();
			var solCheck:SolutionCheckProcess = new SolutionCheckProcess(this, true);
			rules.addToQueue(solCheck);
		}
	}
	...
	{		
					...
					for (r in 0...columnArray.length)
					{
						if (columnArray[i + r] == null || (columnArray[i + r].mainArray[o] == null || columnArray[i + r].mainArray[o] != null && columnArray[i + r].mainArray[o].name != pieceName))
						{
							if (pieceArray.length >= 3)
							{
								...
								var processArray:Array<PuzzlePiece> = new Array();
								
								for (i in 1...pieceArray.length)
								{
									removeArray.push(pieceArray[i]);
									
									processArray.push(pieceArray[i]);
								}
		...
		for (i in 0...removeArray.length)
		{
			...
			//trace("Removearray length: " + removeArray.length);
